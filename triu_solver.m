...
if n < opts.threshold
    ...
    if opts.recursive > 0
        solver_handle = @(A_,b_,opts_)(triu_solver(A_,b_,opts_));
    else
        solver_handle = @(A_,b_,opts_)(naive_solver(A_,b_,opts_));
    end
    for i = n : -chunk_size :1
        if i - chunk_size <= 0
            b(1:i,:) = solver_handle(A(1:i,1:i),b(1:i,:),opts);
        else
            b(i-chunk_size+1:i,:) = solver_handle(A(i-chunk_size+1:i,i-chunk_size+1:i),b(i-chunk_size+1:i,:),opts);
